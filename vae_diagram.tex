...
\node[vectornode] (mean) [below=of dummy] {$\mu(X)$};
\node[vectornode] (std) [above=of dummy] {$\Sigma(X)$};
\node[vectornode] (latent) [right=1.5cm of dummy] {\Vectorstack{{z_1} {\vcdots} {z_n}}};
\node[networknode] (decoder) [right=1.5cm of latent][label=below:{\mbox{$P(X|z)$}}]
...
\draw[->] (encoder.east) |- ++(.5,0.3) -- ++(0, 1) |- (std.west);
\draw[->] (encoder.east) |- ++(.5,-0.3) -- ++(0, -1) |- (mean.west);
