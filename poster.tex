...
% The rule could possibly be done in tikz
{
  ...
  \begin{center}
    \begin{minipage}{0.99\linewidth}
      {\LARGE
      Numerosity, the number of objects in a set, is a basic property of a given visual scene. Inspired by \citet{stoianov2012} we propose an unsupervised generative model to learn visual numerosity representations from natural and synthetic image datasets. Specifically, we use a variational autoencoder (VAE) for encoding and reconstructing training images. The underlying principle is that numerosity is a key characteristic in the images and the network learns to encode visual numerosity in the latent representation.\par} % \par changes a lot
      \end{minipage}
  \end{center}
  \vspace*{10pt}
...
\innerblock{}{\boldblue{Subitizing} is a perceptual skill shared by many animals allowing them to immediately percieve the numerosity of small sets. 
...
%   	- spontaneous, etc.
\innerblock{}{We explore the emergence of the perceptual skill of subitizing in VAEs using \boldblue{natural images} from the \boldblue{Salient Object Subtizing Dataset} \citep{zhang2016salient} to train our model. 
