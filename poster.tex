...
% leave it
\innerblock{}{\boldblue{VAEs} are a generative algorithm that perform \boldblue{unsupervised} representation learning. Instead of mapping data samples $X$ to a deterministic latent representation as in conventional autoencoders, VAEs learn a posterior distribution \mbox{$Q(z \mid X)$}. Inputs can be reconstructed by sampling latent vectors $z$ from the posterior distribution and passing them through a decoder network. We parameterize the encoder and decoder network with deep CNNs.}
\colorlet{innerblockbodybgcolor}{eccvgrayblue}
