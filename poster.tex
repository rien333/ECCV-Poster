...
titletotopverticalspace=40pt, titletoblockverticalspace=20pt}{
...
% The rule could possibly be done in tikz
{
  ...
  \begin{center}
    \begin{minipage}{0.99\linewidth}
      ...
      Numerosity, the number of objects in a set, is a basic property of a given visual scene. Inspired by \citet{stoianov2012} we propose an unsupervised generative model to learn visual numerosity representations from natural and synthetic image datasets catered to instance counting within the subitizing range. Specifically, we employ a hierarchically organized convolutional variational autoencoder (VAE) tasked with encoding and reconstructing training images. Given that numerosity is a key characteristic in the images, the underlying principle is that the network learns to encode visual numerosity in the latent representation.\par} % \par changes a lot
      \end{minipage}
  \end{center}
  \vspace*{10pt}
...
% I guess it doesn't really capture the "very little" parth tho
...
When the \boldblue{subitizing range} of 1 -- 4 instances is exceeded, other cogntive mechanisms are employed to arrive at an instance count. }
%% \begin{figure}%
...
%   	- spontaneous, etc.
\innerblock{}{We explore the emergence of visual number sense in deep networks trained in an unsupervised setting on \boldblue{natural images} from the \boldblue{Salient Object Subtizing Dataset} \citep{zhang2016salient}. The complexity of natural visual scenes helps capture the abstract nature of number sense.
% Similair to previous work, we hope to show that (emergence/visual number sesnse).
}
}
...
\colorlet{innerblockbodybgcolor}{white}
% Too long probably, maybe reduce to a figure
\innerblock{}{\boldblue{VAEs} are a generative algorithm that can be trained in a unsupervised manner. Instead of mapping data samples $X$ to a deterministic latent representation as in conventional autoencoders, VAES learn to a posterior distribution \mbox{$Q(z \mid X)$}. Inputs can be reconstructed by sampling latent vector $z$ from the posterior distribution and passing it through a decoder network. We parameterize the encoder and decoder network with deep CNNs, enabling training by stochastic gradient descent. }
\colorlet{innerblockbodybgcolor}{eccvgrayblue}
...
\colorlet{innerblockbodybgcolor}{white} 
% move CNN stuff upwards?
\innerblock{}{To make sampling feasible posterior distribution \mbox{$Q(z | X)$} is (Guassian stuff). P(z) is often implemented as...) }
}
\end{columns}
...
\innerblock[bodyinnersep=25pt]{}{
  ...
  \begin{minipage}{0.494\linewidth}
    ...
    %% \hspace*{45pt}
    \begin{tikzfigure}
      \hspace*{15pt}\includegraphics[width=0.23\linewidth]{syn1}
      \includegraphics[width=0.23\linewidth]{syn2}
      \includegraphics[width=0.23\linewidth]{syn3}
      \includegraphics[width=0.23\linewidth]{syn4}
    \end{tikzfigure}%
 \end{minipage}%
...
{A mixture of natural images and \boldblue{synthetic data} was used to augment the SOS dataset. Images are synthesized by cut-pasting objects onto natural image backgrounds with various random image transformations applied. Although unrealistic looking, ... }
%% \vskip 0.11em
\end{multicols}
}
}
...
\colorlet{innerblockbodybgcolor}{eccvgrayblue}
\innerblock{}{\vspace*{120pt} \boldblue{VAE + softmax.} \blindlist{enumerate}[2] \vspace*{120pt}}
}
\column{0.565}
...
\innerblock{}{
  ...
  \begin{tikzfigure}
    \vspace*{-10pt}
    \includegraphics[width=0.49\linewidth]{NN}
    \includegraphics[width=0.49\linewidth]{NA}
  \end{tikzfigure}
}
\colorlet{innerblockbodybgcolor}{eccvgrayblue}
\innerblock{}{
